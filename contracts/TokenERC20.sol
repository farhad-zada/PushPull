...
contract Token is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    modifier isAdmin() {
        require(admins[msg.sender], "Token: not admin");
        _;
    }
    ...
    function mint(address to, uint256 amount) public isAdmin {
        require(
            this.totalSupply() + amount <= 5_000_000_000 * 10 ** 18,
            "Token: max supply"
        );
    ...
    function setTransferFeePercent(uint256 fee) public onlyOwner {
    ...
    function setWhitelistBatch(
        ...
        require(_addresses.length == status.length, "Token: length mismatch");
